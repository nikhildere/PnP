//Mdlz Template 03 Javascript file
...
iconsrc = "/SiteAssets/vNext/Team/Templates/Mdlz_T03/Images/T03_banner.png"
} ; // end if-else
...
icon[0].setAttribute("src" , "/SiteAssets/vNext/Team/Templates/Mdlz_T03/Images/MDLZSiteIconTransparent.png") ;
...
if (text.indexOf("SHOWBREADCRUMB") == -1) {
	...
	if(ieVersion == 7)
	{
	    $(".ms-breadcrumb-anchor").css("cssText", "margin-left: 800px !important;");
		if($("#zz12_TopNavigationMenu ul[id*='zz13_RootAspMenu']").length)
			$("#DeltaTopNavigation ul[id*='RootAspMenu']").css("cssText","margin-left: -684px !important;");
		else
			$("#DeltaTopNavigation ul[id*='RootAspMenu']").css("cssText","margin-left: -684.5px !important;");
	} 
}
...
//This function assigns background color to root navigation bar; Needed in case of multicolor navigation
//Whole Navigation bar to have color of last nav item bg color.
function fixTopNavBarBgColor()
{
//Publishing 
if ($("#DeltaTopNavigation ul").length > 1) {
var NavItemsCount = $('#DeltaTopNavigation div ul ul li.static').length;

var bckcolor= $('#DeltaTopNavigation div ul ul li.static').last().css( "background-color")
$('#DeltaTopNavigation div ul ul li.static').last().css("border-right","0px");
$('#DeltaTopNavigation div ul ul li.static').last().parent().css( "background-color",bckcolor)
}
else { //Non Publishing 
var NavItemsCount = $('#DeltaTopNavigation div ul li.static').length;

 var g = $('#DeltaTopNavigation div ul li.ms-listMenu-editLink').length;
                if (g > 0) {
var bckcolor= $('#DeltaTopNavigation div ul li.static:nth-last-child(2)').css( "background-color")
bckcolor = bckcolor  + " !important";
$('#DeltaTopNavigation div ul li.static:nth-last-child(2)').css("border-right","0px");
$('#DeltaTopNavigation div ul li.static:nth-last-child(2)').parent().css( "background-color",bckcolor)
}
        else {
var bckcolor= $('#DeltaTopNavigation div ul li.static').last().css( "background-color")
$('#DeltaTopNavigation div ul li.static').last().css("border-right","0px");
$('#DeltaTopNavigation div ul li.static').last().parent().css( "background-color",bckcolor)
